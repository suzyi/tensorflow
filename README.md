...
+ **uninstall cuda Toolkit and driver**. Before installing CUDA, uninstall previously installation that could conflict. Even though you hadn't previously have the cuda installed in your system, you can perform the uninstall step and this will not affect your system. By default, cuda was installed in the directory `/usr/local/cuda-X.Y/`. To uninstall it, execute `sudo /usr/local/cuda-X.Y/bin/uninstall_cuda_X.Y.pl` and then uninstall a Driver runfile installation by `sudo /usr/bin/nvidia-uninstall`. Uninstall RPM/Deb installation by `sudo apt-get --purge remove <package-name>`.
