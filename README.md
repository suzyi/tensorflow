...
+ initiate global variables. `init = tf.global_variables_initializer()` or `init = tf.initialize_all_variables()`
+ **tf.reshape(), tf.stack() [(notebook)](https://github.com/suzyi/tensorflow/blob/master/tf/reshape_stack.ipynb)**
+ **tf.concat() [(notebook)](https://github.com/suzyi/tensorflow/blob/master/tf/concat.ipynb)**
