...
+ **[tf.layer.dense(inputs,units,activation=none,use_bias=none)](https://github.com/tensorflow/tensorflow/blob/r1.2/tensorflow/python/layers/core.py)**. `units` is the dimensionality of output space. Build a densely connected network. Default activation is linear function. Default use a bias.
